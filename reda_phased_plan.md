...
- New scheduled function [`scheduled-eod-check`](supabase/functions/scheduled-eod-check/index.ts) wired to Supabase Cron. (Initially `0 19 * * *`; current schedule is `59 22 * * *` UTC = 23:59 Lagos — see iteration log for the revision history.)
...
- **Cron schedule manual step:** updated Supabase dashboard cron from `0 19 * * *` to `0 20 * * *` (21:00 Lagos / 20:00 UTC). *(Later revised — current schedule is `59 22 * * *` UTC = 23:59 Lagos. The 21:00 mention here is historical.)*
...
**Per-client EOD auto-cancel for soft-fails (2026-05-26)**
- Shalom's operational reality: when their customer doesn't engage on the scheduled day, they don't want the order rolling forward and accumulating retry history. They handle the customer relationship themselves and prefer the order to die as a `failed_delivery`.
- Generalized as a per-client policy (toggleable for any client) — Shalom is just the first to opt in. New column `clients.auto_cancel_soft_fails boolean not null default false`, mirroring the existing `max_charge_per_delivery` pattern.
- **Server** ([scripts/per-client-eod-soft-fail-cancel.sql](scripts/per-client-eod-soft-fail-cancel.sql), single paste-able transaction):
  1. Add the column with a COMMENT documenting which statuses it affects (the customer-unreachable subset only — not customer deferrals like `tomorrow`/`postponed`, not in-transit like `picked_up`/`waybilled`).
  2. Extend `update_client(...)` with `p_auto_cancel_soft_fails boolean default null` — same coalesce semantics as `max_charge_per_delivery` (null = leave unchanged). Audit log captures old/new value.
  3. Extend `_notify_admins_eod_summary(...)` with `p_policy_cancel_count integer default 0` (back-compatible default) so the admin EOD digest reports both rolled + policy-cancelled counts in one push.
  4. Replace `run_eod_rollover(...)` — added `client_id` to the eligible CTE, spliced a new branch BETWEEN dedup logic and the default `rollover_delivery` call. The branch checks the client policy + status, calls `change_delivery_status(... 'failed_delivery', 'eod_auto_cancel:client_policy')`, increments a new `v_policy_cancels` counter, and `continue`s past the rollover. Uses the existing RPC so audit, sibling coordination, and `notify_delivery_status_change` admin push all fire naturally.
  5. `UPDATE clients SET auto_cancel_soft_fails = true WHERE name = 'Shalom'`.
- **Mobile**:
  - [services/clients.ts](mobile/src/services/clients.ts): `ClientInput.autoCancelSoftFails?: boolean | null` (null = leave unchanged) + `updateClient(...)` forwards as `p_auto_cancel_soft_fails`.
  - [(admin)/catalog/clients/[id].tsx](mobile/app/(admin)/catalog/clients/[id].tsx): new `<Switch>` row labeled "Cancel soft-failed orders at EOD" with helper text explaining which statuses are affected. State + dirty-tracking mirrors `maxCharge`. The save call passes `autoCancelSoftFails` only when the toggle changed (mirrors the maxCharge pattern of "null means leave alone, value means change").
  - [database.gen.ts](mobile/src/types/database.gen.ts): hand-patched ahead of `npm run gen:types` (clients.Row + update_client.Args). `gen:types` will mechanically regenerate the same shape after the SQL paste lands.
- **Tech-debt audit**: no new RPC, no new audience kind, no new push trigger, no new status. One new column on `clients` (mirrors `max_charge_per_delivery`'s exact pattern). One new param on the existing `update_client` RPC. One new branch in `run_eod_rollover` that reuses the existing `change_delivery_status` RPC. No hardcoded "Shalom" in code — only a SQL UPDATE flipping their row's flag. Adding another client tomorrow is a one-tap UI toggle. Mobile typecheck clean.
- **Ship order**: paste SQL → `cd mobile && npm run gen:types` (verify the manual patch matches) → `cd mobile && npx eas update --branch preview --platform android --message "Per-client EOD auto-cancel toggle (Shalom enabled)"` → re-enable the EOD cron in Supabase Dashboard.
...
**Bigger ship day: redaction, intervention auto-thread, postpone-with-date, notified pill, Review badge, duplicate guards (2026-05-27)**

A dozen distinct items landed on top of yesterday's per-client EOD auto-cancel. Grouped by user-visible feature for the changelog; tech-debt audit lives at the bottom.

- **Agent client redaction.** Riders rotate; client relationships are a defensible asset. Vendor name is now hidden across every agent surface — Today list cards (no more `Client · Product` prefix), delivery detail (the *Vendor* card was replaced with a standalone *Call admin / dispatch* CTA), Earnings rows, and *My Stock* rows. `listAgentEarnings` dropped the `clients(name)` join entirely so the field is gone from the wire and the TS type. Today's enforcement is UI + service-layer; `deliveries_safe` and `clients` RLS still expose names server-side, so a network-inspecting agent could still read them — tighten there only if poaching becomes a real incident. Recorded in [reda_system_design_doc.md §2 Key access rules](reda_system_design_doc.md).

- **Postpone requires a date now.** [UpdateStatusSheet](mobile/src/components/sheets/UpdateStatusSheet.tsx) renders a YYYY-MM-DD date input plus `+1 / +2 / +3 / +7` quick chips whenever the picked transition is `postponed`. The Sunday-skip rule already in `_ensure_workday()` continues to apply server-side. `change_delivery_status` got a new `p_new_scheduled_date` param (server ignores it for every status except `postponed`); queue executor + types threaded through. `MarkDeliveredSheet` and `StatusUpdatePanel` pass null because they never postpone.

- **Client-notified tag on each status-history row.** Multiple reps in the loop need a way to mark "I told the customer about this status change" so peers stand down. New SQL paste [scripts/client-notified-tag.sql](scripts/client-notified-tag.sql): `delivery_client_notifications` table keyed by `status_history_id`, denormalized `delivery_id` for fast per-delivery queries, RLS allows ops + the row's assigned agent to SELECT (informational read for the agent). First-tap wins via PK conflict on `status_history_id`. New RPC `mark_client_notified(p_status_history_id)` returns the holder record so the UI shows *"<Name> told the client"* without an extra round-trip. Separate table (not extra columns on `delivery_status_history`) keeps that table's append-only invariant intact — mirrors how `delivery_followups` and `delivery_messages` are factored. Mobile: new service [clientNotifications.ts](mobile/src/services/clientNotifications.ts) with `markClientNotified` + `listClientNotificationsForDelivery` (returns a `Map<status_history_id, ClientNotificationRow>` for O(1) lookup in the HistoryRow renderer). Permission helper `canMarkClientNotified(role)` — ops-only. Each HistoryRow on the delivery detail renders either a green-check *"Client notified · <Name> · <time>"* line (if tagged) or a *"Mark client notified"* pill (if not, ops-only). Pairs with the new list-row pill below.

- **'Notified' pill on the deliveries list (since 2026-05-27).** Surfaces the same tag at glance-distance so peers don't open each delivery to check. `listDeliveries` now resolves the latest `delivery_status_history.id` per delivery in the same sub-query it already runs for the sort (added `id` to the SELECT, tracked `latestHistoryByDelivery` map), then fires one cheap `delivery_client_notifications` lookup keyed on that set of ids and merges `latest_notified: boolean` into each row. Pill renders next to the StatusPill in [DeliveryListRow](mobile/src/screens/deliveries/List.tsx) — `colors.successSoft` background, `check` icon, *Notified* label. Hidden when `latest_notified` is false. Single-row `getDelivery` defaults `latest_notified` to false (the Detail screen renders per-history notification state inline, so the aggregate isn't needed there).

- **Rep dashboard separated from OpsDashboard.** Reps wanted a Recent-activity card on their home; dispatchers don't. New [RepDashboard.tsx](mobile/src/screens/ops/RepDashboard.tsx) split out as a sibling component to [OpsDashboard.tsx](mobile/src/screens/ops/OpsDashboard.tsx). Both reuse a new shared [RecentActivityCard](mobile/src/components/delivery/RecentActivityCard.tsx) component (admin home also adopted it — net code reduction). `OpsBasePath` type narrowed back to `/(dispatcher)` only; `/(rep)/index.tsx` now imports RepDashboard directly.

- **Ops team call: agents page admin/dispatcher/rep, first-accept wins.** When an agent rings the team about a delivery, the call now fans out to every online admin / dispatcher / rep; whoever accepts first owns the call, the rest see *"<Name> answered"* and the overlay dismisses. New [IncomingCallOverlay](mobile/src/components/IncomingCallOverlay.tsx) and [useIncomingCallSubscription](mobile/src/hooks/useIncomingCallSubscription.tsx) coordinate; coordinator in [src/lib/calls/coordinator.ts](mobile/src/lib/calls/coordinator.ts) wires the accept-race semantics. Mobile `services/calls.ts` + `send-notification` Edge Function gained a new team-broadcast audience.

- **Ops can now seed empty message threads.** Previously the server-side `reply_to_delivery` rejected when no prior agent flag existed — the design was "thread starts with the agent's complaint." With reps coordinating proactively, that rule was too strict. [MessageThread](mobile/src/components/delivery/MessageThread.tsx) now renders a *"Message agent"* composer when the thread is empty AND the viewer is ops AND the parent is non-terminal. The RPC contract was relaxed via the same script that introduced the ops-team-call audience; `canPostOnThread` / `canSeedThread` permission helpers carry the rule client-side. The original "agents seed via flag" path remains the dominant case — the empty-state composer is a fallback for ops-initiated coordination.

- **Auto-seed thread when agent picks an intervention-class status.** Previously the only way for an agent to start a thread was the caution-flag icon (FlagDeliverySheet). If they picked `not_answering` (or any other customer-unreachable status) via the regular *Update status* sheet, ops got no message — only a status change. Closed the gap by routing those submits through the existing `flag_delivery_issue` RPC. New constant `STATUS_AUTO_ISSUE` in [delivery-messages.ts](mobile/src/services/delivery-messages.ts) maps the six unreachable statuses to `cant_reach_client`. New queue job kind `flag_delivery` mirrors `change_delivery_status` so agents keep offline resilience. UpdateStatusSheet gained a `autoSeedThreadOnIntervention` prop — only the agent's delivery-detail screen opts in; ops users keep plain status-change semantics (rep picking `not_answering` after a customer call shouldn't seed a message). UI hint: *"This will also message ops so they can help."* The reason field's label switches to *"Note to ops (required/optional)"* when the route will seed a thread. Zero server changes — `flag_delivery_issue` already does status + message atomically.

- **Review tab badge.** Red pill on the *Review* tab in admin / dispatcher / rep bottom nav shows the count of `bot_inbound_messages.status='needs_review'` rows. New `countNeedsReview()` in [services/bot.ts](mobile/src/services/bot.ts) uses Supabase `head: true` so only the row count comes back. New [useNeedsReviewCount](mobile/src/hooks/useNeedsReviewCount.ts) hook polls every 30s plus refreshes on AppState foreground; errors are swallowed so a network blip stalls the value instead of flickering. Wired into [admin _layout.tsx](mobile/app/(admin)/_layout.tsx) (needs-review screen) and [OpsTabsLayout.tsx](mobile/src/screens/ops/OpsTabsLayout.tsx) (review screen, shared by dispatcher + rep). Hidden when count is 0.

- **New delivery: past-date warning + pre-submit duplicate check.** Triggered by an incident where Uzo backdated a manual delivery to yesterday and Damilola never saw it on her Today tab; he assumed it failed and created a second row with a slightly different (typoed) address, slipping past the server-side same-agent sibling guard which is strict about address-normalized match. Two UI-layer fixes, no schema change:
  - [New.tsx](mobile/src/screens/deliveries/New.tsx) now shows an amber *"Scheduled for a past date"* Banner whenever `scheduledDate < todayLagos()`. Soft warning, not a hard block — admins legitimately backfill yesterday's manual entries during reconciliation.
  - New service `findSimilarOpenDeliveries(agentId, customerPhone, productCatalogId, scheduledDate)` queries the `(agent, normalized phone, product, date)` tuple for open (non-terminal, non-deleted) rows. `OPEN_STATUSES` is derived from `STATUS_GROUPS.active ∪ .soft` so future bucket changes carry through. Pre-submit, the form runs this lookup; if any match exists, a cross-platform `confirmAsync` dialog (`Alert.alert` on native, `window.confirm` on web) lists the existing rows by status + address and requires explicit *"Create anyway"*. Pre-check is best-effort: errors are swallowed so a transient network blip can't block legitimate creation (the server-side guard is still the source of truth).
  - The two rows from the actual Damilola incident (`de11f08c…` Festac, `b2ba92e0…` "f") were cleaned up by direct UPDATE: keeper re-dated to today, typo'd row soft-deleted via `deleted_at = now()`.

- **Lint + Prettier sweep across the mobile tree.** CI #5 was the first run with `lint --max-warnings 0` + `format:check` enforced and surfaced 12 lint errors + 138 unformatted files that had drifted over months. Fixed in one pass: `err: any` in catch clauses converted to `err instanceof Error ? err.message : String(err)`; unused imports removed; `eslint-disable` comments tightened on the defensive `react-native-callkeep` require; unescaped apostrophes escaped; `npm run format` applied. One follow-up: Prettier moved an `eslint-disable-next-line` comment off its target in `InboundDetailScreen.tsx` — nudged back to the right line. No semantic changes, purely hygiene. Mobile typecheck + lint + format:check all green going forward.

- **Ops dashboard headline: drop money, collapse to customer chains.** Per Uzo's input — the dispatcher home shouldn't show running revenue/margin numbers; the operational headline is *"how many distinct customers are still open."* [OpsDashboard.tsx](mobile/src/screens/ops/OpsDashboard.tsx) hero replaced the money strip with sibling-aware counts (Active / Closed / Rolled chain counts via `siblingGroupKey`).

- **Tech-debt audit (whole-day rollup).** Net new: one SQL paste (`scripts/client-notified-tag.sql` — table + RPC), one new mobile service file (`clientNotifications.ts`), one new hook (`useNeedsReviewCount.ts`), one new queue job kind (`flag_delivery`, mirrors existing executor pattern), one new shared component (`RecentActivityCard`), one new screen (`RepDashboard.tsx`). No new RPCs beyond `mark_client_notified`; the auto-seed-thread feature reuses the existing `flag_delivery_issue`. `STATUS_AUTO_ISSUE` lives next to `ISSUE_DEFAULT_STATUS` so the inverse-mapping symmetry is visible. `OPEN_STATUSES` derived from `STATUS_GROUPS` so future status additions to the active/soft buckets automatically widen the duplicate check. Mobile typecheck + lint + format:check clean at end of day.

- **Ship order (typical day):** SQL paste → `cd mobile && npm run gen:types` → `cd .. && git push origin main` → `cd mobile && npx eas update --branch preview --platform android --message "<feature summary>"`. EOD cron remained paused; will re-enable after the auto-cancel policy gets one more day of observation.

**Warehouse role: own the physical stock flow (2026-05-27)**

Until today the `warehouse` role could log in but only **view** stock — every intake / transfer / adjustment routed through admin (`is_admin()` gate inside both `create_stock_adjustment` and `create_stock_transfer`). The role's logical domain is the physical warehouse location (one identity per place — Shomolu warehouse today; humans on the floor share the login). Mapping the capability set to that reality:

- **Server-side guard relaxed via single SQL paste** ([scripts/warehouse-stock-ops.sql](scripts/warehouse-stock-ops.sql)): the two RPCs got a new permission preamble — `admin → all paths (unchanged)` OR `warehouse → scoped paths`, else 42501. For `create_stock_adjustment`, warehouse may pass `bulk_intake / loss / theft / damaged / found` but only with `p_agent_id = auth.uid()` (own-holdings only). `correction` stays admin-only — the books-override escape hatch keeps a single accountability anchor. For `create_stock_transfer`, warehouse may run `warehouse_issue` (from = self) or `warehouse_return` (to = self); `transfer` (agent → agent) stays admin-only.
- Defense-in-depth: new RLS policy `stock_adj_insert_warehouse_self` mirrors the loosened RPC. `stock_adj_all_admin` stays — admin still has ALL via it. Direct INSERTs (no RPC) by warehouse are constrained to `agent_id = auth.uid()`.
- **Push fan-out**: `tg_notify_pickup_needed` now fires both `audience='admins'` (existing) AND `audience='warehouse_pickup'` (existing helper in `send-notification/index.ts` — filters `users.role='warehouse'`). Warehouse staff see pickup-needed signals on their phones because they can now issue from the warehouse app directly. Two `http_post` calls per event, cheaper than adding a third audience type.
- **Mobile — UI lift, no duplicated screens**: the three admin stock action screens (`receive.tsx`, `transfer.tsx`, `adjust.tsx` — ~1100 lines combined) moved into shared components at [mobile/src/screens/stock/{Receive,Transfer,Adjust}.tsx](mobile/src/screens/stock/) taking a `scope: 'admin' | 'warehouse'` prop. Admin route files became 3-line re-exports rendering with `scope='admin'`; new warehouse route files render the same components with `scope='warehouse'`. The `scope` branch is the only role-aware logic in the shared screens — Receive hides the destination picker (locks to caller); Transfer filters out the `transfer` reason and locks the warehouse side; Adjust filters out the `correction` reason and locks the holder picker. Admin output must be byte-identical to before; verified by inspecting both routes after the move.
- **Permission helpers** ([mobile/src/lib/permissions.ts](mobile/src/lib/permissions.ts)): replaced the single `canAdjustStock(role) === 'admin'` with a precise set — `canAdjustAnyStock`, `canAdjustOwnStock`, `canReceiveStock`, `canDoWarehouseTransfer`, `canTransferAgentToAgent`, `canCorrectStock`. Each has a `Server anchor:` docstring naming the SQL guard branch so server + client stay in lock-step.
- **Warehouse route group** ([mobile/app/(warehouse)/](mobile/app/(warehouse)/)): `_layout.tsx` gained three `<Tabs.Screen ... href: null />` entries so the action screens are routable but invisible in the bottom tab bar. Dashboard ([index.tsx](mobile/app/(warehouse)/index.tsx)) gained an action button row at the top (Receive stock primary, New transfer + Adjustment secondary) mirroring admin's Stock-screen pattern. Buttons gated by the new permission helpers — admin renders the same row from the same canRecipes; warehouse renders the same row when logged into Shomolu. Notification tap-routing in [notifications.ts](mobile/src/lib/notifications.ts) for `data.route === 'stock'` now maps `warehouse → /(warehouse)` (was admin-only before).
- **Identity model clarified at the role level**: per the user's direction, the warehouse-role user IS the physical location, not a per-human identity. Martha (created earlier 2026-05-27 as a separate `martha@reda.dev` warehouse user) was deleted; she'll log into Shomolu warehouse like the rest of the floor staff. Doc: [reda_system_design_doc.md §2](reda_system_design_doc.md) Warehouse paragraph rewritten to reflect this.
- **Tech-debt audit**: no new role, no new column, no new audience, no new RPC. The `warehouse_pickup` audience already existed. The shared screens are a strict net reduction in surface (admin's 3 route files went from full bodies to 3 lines each; offsetting 3 new files of similar 3-line shape under `(warehouse)/`; the shared screens themselves carry one new conditional branch each). Mobile typecheck / lint / format clean.
- **Doc updates**: [reda_prd.md §5.10](reda_prd.md) — *Role access* paragraph rewritten as a 5-role × 2-axis table (reads / writes), and the *Screens* bullet gained a "Warehouse Stock screen" entry that documents the `scope='warehouse'` rendering. [reda_system_design_doc.md §2](reda_system_design_doc.md) — Warehouse paragraph extended.
- **Ship order**: paste `scripts/warehouse-stock-ops.sql` → `cd mobile && npx tsc --noEmit` (verify the refactor compiles) → push to main → `cd mobile && npx eas update --branch preview --platform android --message "Warehouse can perform receive / issue / return / own-shrinkage stock ops"`. Manual admin regression smoke before letting Shomolu loose on real intake.

**Bot pipeline reliability bundle (2026-06-03)** — five fixes shipped in one day, all triggered by an investigation into ~96 stuck `needs_review` rows.

1. **bot-parse-message v3 — merge contractor + LLM instead of discard.** Prior `extractContractorParse` discarded the entire contractor `parsed` block if any of `product_name`/`raw_address`/`customer_phone` was empty, then rebuilt from a Gemini extraction. That threw away the contractor's smarter field-by-field knowledge: e.g. contractor `product_name="Perfume"` correctly matched Original Buy's catalog, but after discard the LLM took "OUD AL LAYL BROWN SINGLE WITH OIL 2515-U" (the SKU header) and got 0 candidates. Replaced with a merge: keep contractor's good fields, fill ONLY the empty ones from the LLM. Cleared 52 of today's stuck rows on first replay.

2. **bot-parse-message swapped Gemini → OpenRouter + gpt-4.1-mini.** Order-field extraction now goes through `openai/gpt-4.1-mini` via `https://openrouter.ai/api/v1/chat/completions` with strict json_schema response format. Direct `generativelanguage.googleapis.com` calls removed from this function. `OPENROUTER_API_KEY` is the only secret needed. Prompt also extended with phone-as-name fallback ("if no name in message, use phone digits as customer_name") so the contractor's existing convention works on the LLM path too.

3. **normalize-address swapped Gemini → OpenRouter + gemini-2.5-flash.** Same model brain, but routed through OpenRouter so we're not capped at Google's free-tier 20-req/day. Prior to this, the address pipeline silently 429'd on every request past 20 each morning — all subsequent addresses landed `confidence='none'` and went to `needs_review`. After the swap, 13 of today's 13 address-failing residuals cleared (12 successfully matched, 1 correctly rejected as out-of-Lagos). Cleanup of `--no-verify-jwt` deploy footgun also discovered here — without that flag, `supabase.functions.invoke('normalize-address', ...)` from inside `bot-parse-message` silently 401s.

4. **bot_create_delivery dedup-on-same-agent (replaces 2026-05-19 spread-on-dupe behavior).** When contractor re-forwards an order naming an agent who already holds an open sibling, function now raises `P0001` with hint `{kind:'duplicate_same_agent', existing_delivery_id, agent_id}` instead of silently nulling the assignment and letting auto-assign hand the row to a different agent. `bot-parse-message` catches the hint and marks the inbound row `status='duplicate'` pointing at the canonical delivery — no new row, no second agent. Found 11 phantom races already alive in production from the prior behavior (Kenneth 5, Anjola 3, Audrey 2, Queen Favour 1); cancelled them via [scripts/cleanup-phantom-race-rows.sql](scripts/cleanup-phantom-race-rows.sql). Side incident: the cascade trigger took down 12 intended-agent canonicals when we cancelled the phantoms (Iya Ayo 10, Olawale 1, Mary 1). Restored via cancelled → pending. The cascade behavior is "correct" given `cancelled`'s order-side semantics; the fix is `agent_cancelled` below.

5. **`agent_cancelled` status — shipped 2026-06-03.** New non-cascading terminal status, label *"Not my delivery"*. Distinguishes "this specific agent's instance is closed but the order is still live" (`agent_cancelled`, non-cascading — what we needed for phantom cleanup) from "the order itself reached a terminal state" (every other terminal status — `delivered`, `cancelled`, `failed_delivery`, `no_product`, `abandoned`). Those all keep cascading because they describe order-level outcomes. One row in `delivery_status_defs`, transitions generated dynamically from every non-terminal status, and one added exclusion in `tg_handle_sibling_coordination` alongside the existing `rolled_over` skip. Mobile picker shows a status-specific warning banner explaining the non-cascade semantics. Self-serve by the assigned agent with required reason; agent can self-revert their own pass. Per-row cleanups now use this status instead of soft-delete. SQL: [scripts/add-agent-cancelled-status.sql](scripts/add-agent-cancelled-status.sql); smoke: [scripts/smoke-agent-cancelled.sql](scripts/smoke-agent-cancelled.sql); full design in [reda_system_design_doc.md §3.5](reda_system_design_doc.md).

Ship order across the bundle: SQL paste (`fix-bot-create-delivery-no-phantom-race.sql`) → smoke (`smoke-bot-create-delivery-no-phantom-race.sql`) → edge function deploy (`bot-parse-message`, then `normalize-address`, **always with `--no-verify-jwt`**) → live replay of stuck rows. Today's `bot_inbound_messages` went from 96 stuck → 1 (correctly flagged as out-of-Lagos).
