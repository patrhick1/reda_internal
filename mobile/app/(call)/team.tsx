...
export default function TeamScreen() {
  ...
  const relatedDeliveryId =
    typeof related_delivery_id === 'string' && related_delivery_id.length > 0
      ? related_delivery_id
      : null;
  ...
  const onCall = useCallback(
    async (target: CallableUser) => {
      ...
      try {
        ...
        if (msg.includes('ringing call')) {
          Alert.alert(
            'Already on a call',
            'You or this person already has a call ringing. Try again in a moment.',
          );
        } else {
    ...
    [callingId, router, relatedDeliveryId],
  );
  ...
  return (
    <View style={{ flex: 1, backgroundColor: colors.surface }}>
      <AppBar
        title="Team"
        subtitle={
          canPlaceCall() ? 'Tap a name to start a call' : 'Calls available on the mobile app'
        }
        onBack={() => router.back()}
      />
      {!canPlaceCall() ? (
      ...
      {loading ? (
        ...
        <Empty
          ...
          sub="Once admin adds users you can call them here."
        />
      ) : (
        <FlatList
