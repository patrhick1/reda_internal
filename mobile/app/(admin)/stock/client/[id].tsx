...
import {
  ActivityIndicator, FlatList, RefreshControl, Share, Text, View,
} from 'react-native';
import { useFocusEffect, useLocalSearchParams, useRouter } from 'expo-router';
import { useAsync } from '@/hooks/useAsync';
import { listCurrentStock, groupByClient, type ClientProductTotal, type ClientStockGroup } from '@/services/stock';
...
export default function ClientStockDetail() {
  const router = useRouter();
  const { id, name } = useLocalSearchParams<{ id: string; name?: string }>();

  const stockQ = useAsync(() => listCurrentStock(), []);
  const productsQ = useAsync<Product[]>(() => (id ? listActiveProductsByClient(id) : Promise.resolve([])), [id]);
  ...
  // Merge the client's full active catalog with the current_stock totals so
  // products with zero on-hand still show up (so Uzo can see what's out of
  // stock). current_stock only emits non-zero rows, hence the merge.
  ...
  return (
    <View style={{ flex: 1, backgroundColor: colors.surface }}>
      ...
      <FlatList
        ...
        ListHeaderComponent={
          products.length > 0 ? (
            <Card style={{ marginBottom: 8 }}>
              ...
              <Text style={{ fontFamily: fonts.medium, fontSize: 12, color: colors.textSecondary, marginTop: 4 }}>
        ...
        ListEmptyComponent={
          stockQ.error || productsQ.error
            ? <Empty icon="alert" title="Could not load" sub={stockQ.error ?? productsQ.error ?? ''} />
            : (stockQ.loading && !stockQ.data) || (productsQ.loading && !productsQ.data)
              ...
              : <Empty icon="warehouse" title="No products" sub={`${clientName} has no active products in the catalog.`} />
        }
      />
      ...
      <View style={{
        ...
        <Button variant="emphasis" full icon="share" onPress={onShare} disabled={products.length === 0}>
          Share with client
        </Button>
      </View>
    </View>
        ...
        <View style={{ flex: 1 }}>
          ...
          {out ? (
            <View style={{
              ...
              <Text style={{ fontFamily: fonts.bold, fontSize: 11, color: colors.red, letterSpacing: 0.3 }}>
                Out of stock
              </Text>
            </View>
          ) : (
