...
export default function ClientReconcileDetail() {
  ...
  const totals = useMemo(() => {
    let customerOwed = 0, paid = 0, redaFee = 0;
    for (const r of rows) {
        ...
        ListHeaderComponent={
          <Card style={{ marginBottom: 8 }}>
            ...
            <View style={{ marginTop: 14, gap: 6 }}>
              ...
              <SmallRow label="Outstanding"   value={formatNaira(totals.outstanding)} accent={totals.outstanding > 0 ? colors.red : undefined} />
        ...
        ListEmptyComponent={
          detailQ.error
            ...
            : detailQ.loading
              ...
              : <Empty icon="wallet" title="No deliveries" sub="No delivered rows for this client in this date range." />
        }
      />
      ...
      <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12 }}>
        <View style={{ flex: 1 }}>
          ...
          <Text style={{ fontFamily: fonts.medium, fontSize: 12, color: colors.textSecondary, marginTop: 2 }} numberOfLines={1}>
            {product}{qty != null ? ` · ${qty} units` : ''}
          </Text>
          <Text style={{ fontFamily: fonts.medium, fontSize: 12, color: colors.textSecondary, marginTop: 2 }} numberOfLines={1}>
            {loc} · {date}{agent ? ` · ${agent}` : ''}
          </Text>
        </View>
        <View style={{ alignItems: 'flex-end' }}>
          <Text style={{ fontFamily: fonts.extrabold, fontSize: 16, color: remit >= 0 ? colors.success : colors.red, letterSpacing: -0.2 }}>
            {formatNaira(remit)}
          </Text>
...
function SmallRow({ label, value, accent }: { label: string; value: string; accent?: string }) {
  return (
    <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
      ...
      <Text style={{ fontFamily: fonts.bold, fontSize: 13, color: accent ?? colors.black }}>{value}</Text>
    </View>
  );
}
...
function MicroRow({ label, value }: { label: string; value: string }) {
  return (
    <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
      ...
      <Text style={{ fontFamily: fonts.semibold, fontSize: 11, color: colors.textSecondary }}>{value}</Text>
    </View>
  );
}
