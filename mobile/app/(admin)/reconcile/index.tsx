...
import {
  ActivityIndicator, Alert, FlatList, Platform, Pressable, RefreshControl, Share, Text, View,
} from 'react-native';
import { useFocusEffect, useRouter } from 'expo-router';
import { useAsync } from '@/hooks/useAsync';
import {
  listAgentEarningsSummary, listClientRemit, runEodRollover,
  type AgentEarningsRow, type ClientRemitRow,
} from '@/services/reconciliation';
import { AppBar, Avatar, Button, Card, Empty, FilterChips, Hint, Icon, Input, Tabs } from '@/components/ui';
...
function presetRange(p: Preset): { from: string; to: string } | null {
  switch (p) {
    case 'today':     return { from: todayLagos(),     to: todayLagos() };
    case 'yesterday': return { from: yesterdayLagos(), to: yesterdayLagos() };
    case 'last7':     return { from: daysAgoLagos(6),  to: todayLagos() };
    ...
    <View style={{ flex: 1, backgroundColor: colors.surface }}>
      ...
      <View style={{ paddingHorizontal: 16, paddingTop: 12 }}>
        <Hint id={HINTS.HELP_ICON_DISCOVERY} title="Tip — In-app help">
          See the <Text style={{ fontFamily: fonts.bold }}>?</Text> in the top-right? Tap it on any screen to read the help for that screen. Full guide also lives in Profile → Help &amp; support.
        </Hint>
      </View>
      ...
      <View style={{ flexDirection: 'row', gap: 12, paddingHorizontal: 16 }}>
        <View style={{ flex: 1 }}>
          <Input label="From" value={from} onChange={setFrom} autoCapitalize="none" autoCorrect={false} placeholder="YYYY-MM-DD" />
        </View>
        <View style={{ flex: 1 }}>
          <Input label="To"   value={to}   onChange={setTo}   autoCapitalize="none" autoCorrect={false} placeholder="YYYY-MM-DD" />
        </View>
      </View>
...
function ClientsList({
  state, openId, setOpenId, onOpenClient,
}: {
  ...
  return (
    <FlatList
      ...
      refreshControl={<RefreshControl refreshing={state.loading && !!state.data} onRefresh={state.reload} tintColor={colors.black} />}
      ListHeaderComponent={
        <Card style={{ marginBottom: 12 }}>
          <Text style={kicker}>Total remit owed</Text>
          <Text style={{ fontFamily: fonts.extrabold, fontSize: 36, color: colors.black, letterSpacing: -1, marginTop: 4 }}>
      ...
      ListEmptyComponent={
        state.error
          ...
          : state.loading
            ...
            : <Empty icon="wallet" title="Nothing to remit" sub="No delivered rows in this date range." />
      }
    />
  );
}
    ...
    <FlatList
      ...
      ListHeaderComponent={
        <Card style={{ marginBottom: 12 }}>
          ...
          <Text style={{ fontFamily: fonts.extrabold, fontSize: 36, color: colors.success, letterSpacing: -1, marginTop: 4 }}>
            {formatNaira(total)}
          </Text>
          <Text style={{ fontFamily: fonts.medium, fontSize: 13, color: colors.textSecondary, marginTop: 2 }}>
      ...
      ListEmptyComponent={
        state.error
          ...
          : state.loading
            ? <View style={{ padding: 60, alignItems: 'center' }}><ActivityIndicator color={colors.black} /></View>
            : <Empty icon="users" title="No agent earnings" sub="No delivered rows in this date range." />
      }
    />
  );
}
...
function SummaryTab({
  clients, agents, loading, rangeLabel,
}: {
  ...
  const totals = useMemo(() => {
    ...
    return {
      deliveries, customerOwed, customerPaid, outstanding,
      redaFee, remitToClients, agentPayments, margin,
    };
  }, [clients, agents]);
    ...
    <View style={{ flex: 1, padding: 16 }}>
      <Card>
        ...
        <View style={{ marginTop: 16, gap: 10 }}>
          ...
          <SummaryRow label="Outstanding"       value={formatNaira(totals.outstanding)} accent={totals.outstanding > 0 ? colors.red : undefined} />
          ...
          <View style={{ height: 1, backgroundColor: colors.border, marginVertical: 6 }} />
          <SummaryRow label="Reda margin"       value={formatNaira(totals.margin)} accent={colors.success} bold />
        </View>
      </Card>
...
function ExpandableRow({
  isOpen, onToggle, onLongPress, onActionPress, subjectKind, name, countLabel, amount, amountLabel, amountColor, extra,
}: {
  isOpen: boolean;
  onToggle: () => void;
  onLongPress?: () => void;
    ...
    <Card dense style={{ padding: 0 }}>
      ...
      >
        {subjectKind === 'agent' ? (
          ...
          <View style={{
            width: 36, height: 36, borderRadius: 10, backgroundColor: colors.black,
        ...
        <View style={{ alignItems: 'flex-end' }}>
          <Text style={{ fontFamily: fonts.extrabold, fontSize: 16, color: amountColor, letterSpacing: -0.2 }}>
            {formatNaira(amount)}
          </Text>
      ...
      {isOpen ? (
        <View style={{ borderTopWidth: 1, borderTopColor: colors.border, backgroundColor: colors.surfaceAlt, padding: 14 }}>
          {extra.map((e, i) => (
            <View key={i} style={{ flexDirection: 'row', justifyContent: 'space-between', paddingVertical: 4 }}>
              <Text style={{ fontFamily: fonts.medium, fontSize: 13, color: colors.textSecondary }}>{e.label}</Text>
              <Text style={{ fontFamily: fonts.bold, fontSize: 13, color: colors.black }}>{e.value}</Text>
            </View>
          ))}
          {onActionPress ? (
