...
export function SameCustomerPayReview({ deliveryId }: { deliveryId: string }) {
  ...
  return (
    <Card style={{ padding: 16, gap: 12 }}>
      <Text style={{ fontFamily: fonts.bold, color: colors.black }}>Same-customer pay review</Text>
      <Banner tone="info">
        {live
      ...
      {pay ? (
        <View style={{ gap: 8 }}>
          ...
          <Text>
            Reported time:{' '}
            {pay.reported_occurred_at ? formatDateTime(pay.reported_occurred_at) : 'Not supplied'}
          </Text>
      ...
      {editing ? (
        <View style={{ gap: 12 }}>
          <Text>
            Confirm the day the delivery actually happened in Lagos, using the available evidence.
          </Text>
          <DateField
