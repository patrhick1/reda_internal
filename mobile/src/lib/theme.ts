...
export const STATUS_META: Record<string, { label: string; tone: Tone; desc: string }> = {
  ...
  deferred_to_client: { label: 'Deferred to client', tone: 'gray', desc: 'Returned to vendor to handle' },
...
export const STATUS_GROUPS: Record<'active' | 'soft' | 'done' | 'closed', string[]> = {
  ...
  soft:   [
    'not_answering', 'number_busy', 'switched_off',
    'not_connecting', 'not_around', 'will_call_back', 'not_available',
    'tomorrow', 'postponed', 'follow_up',
    'picked_up', 'waybilled',
  ],
  done:   ['delivered'],
  closed: ['cancelled', 'failed_delivery', 'unserious', 'no_product', 'abandoned', 'deferred_to_client', 'rolled_over'],
};
...
export const TONE_PALETTE: Record<Tone, { bg: string; soft: string; text: string; softText: string }> = {
  ...
  amber: { bg: colors.warning, soft: colors.warningSoft, text: colors.white, softText: colors.warningDark },
  green: { bg: colors.success, soft: colors.successSoft, text: colors.white, softText: colors.successDark },
