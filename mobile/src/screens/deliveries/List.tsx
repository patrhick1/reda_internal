...
import {
  AppBar, Avatar, Card, Empty, FAB, FilterChips, Icon, Input, StatusPill,
} from '@/components/ui';
...
export function DeliveriesList({ basePath }: { basePath: BasePath }) {
  ...
  const listFilters = useMemo(() => {
    switch (datePreset) {
      case 'today':     return { date: todayLagos() };
      case 'yesterday': return { date: yesterdayLagos() };
      case 'custom':    return { date: customDate };
      case 'all':       return { allDates: true };
    }
  ...
  const all = useMemo(() => {
    ...
    if (nameNeedle) rows = rows.filter((d) => (d.customer_name ?? '').toLowerCase().includes(nameNeedle));
    return rows;
  ...
  return (
    <View style={{ flex: 1, backgroundColor: colors.surface }}>
      ...
      />
      <View style={{ backgroundColor: colors.white, paddingTop: 12, borderBottomWidth: 1, borderBottomColor: colors.border }}>
        <FilterChips
        ...
        renderItem={({ item }) => {
          ...
          return (
            <DeliveryListRow
              ...
              followup={claim}
              onPress={() => router.push({
                pathname: `${basePath}/deliveries/[id]` as `/(admin)/deliveries/[id]` | `/(dispatcher)/deliveries/[id]` | `/(rep)/deliveries/[id]`,
                params: { id: item.id ?? '' },
              })}
            />
          );
        ...
        ListEmptyComponent={
          error
            ...
            : loading
              ...
              : <Empty icon="package" title="Nothing here" sub={emptySubtitle(datePreset, customDate, agents.find((a) => a.id === agentId)?.display_name ?? null, nameQuery.trim() || null)} />
        }
      />
      <FAB icon="plus" label="Create" onPress={() => router.push(`${basePath}/deliveries/new` as `/(admin)/deliveries/new` | `/(dispatcher)/deliveries/new` | `/(rep)/deliveries/new`)} />
    </View>
  );
}
            ...
            {showFollowup ? (
              <View
                ...
                style={{
                  ...
                  backgroundColor: colors.infoSoft, borderRadius: 999,
                }}
              >
...
function SeparatorH8() { return <View style={{ height: 8 }} />; }
...
function emptySubtitle(preset: DatePreset, customDate: string, agentName: string | null, nameQuery: string | null): string {
  const when =
    preset === 'today'     ? 'today' :
  ...
  switch (preset) {
    case 'today':     return 'No deliveries scheduled for today. New orders show up here when the bot creates them or you add one with the red + button.';
    case 'yesterday': return 'No deliveries scheduled for yesterday.';
    case 'custom':    return `No deliveries scheduled for ${customDate}. Try a different date.`;
    case 'all':       return 'No deliveries yet across all dates. Switch filters above or tap the red + button to create one.';
  }
}
      ...
      <Modal visible={open} transparent animationType="fade" onRequestClose={() => setOpen(false)}>
        ...
        >
          ...
          >
            ...
            <Text style={{
              fontFamily: fonts.bold, fontSize: 13, color: colors.textSecondary,
