...
**Per-client EOD auto-cancel (since 2026-05-26; deferrals added 2026-07-16; same-night postpone close fixed 2026-07-27/28).** Each client also has a boolean `auto_cancel_soft_fails` toggle on the catalog edit screen. When **on**, EOD transitions customer-unreachable statuses (`not_answering`, `not_connecting`, `number_busy`, `switched_off`) plus `tomorrow` and `postponed` to terminal `failed_delivery` instead of rolling/releasing them. A postponed row closes on the night it is postponed even when its requested date is later; the close snaps `scheduled_date` back to the Lagos action day and clears its agent. `not_available` closes to `unserious`, `follow_up` closes to `deferred_to_client`, `will_call_back` keeps the one-roll carry policy, and logistics endpoints (`picked_up`, `waybilled`) are already terminal. Default off; enabled today only for **Shalom** (they handle the customer relationship and do not want orders accumulating retry history). Failed closures for an auto-cancel client are excluded from **To notify** and the rep notification SLA. The status change goes through `change_delivery_status('failed_delivery', 'eod_auto_cancel:client_policy')`, so history, audit, sibling coordination, and admin pushes remain centralized.
...
**Per-client EOD auto-cancel (since 2026-05-26).** Inside the same `run_eod_rollover` flow, after sibling/dedup handling, `auto_cancel_soft_fails` clients close `not_answering`, `not_connecting`, `number_busy`, `switched_off`, `tomorrow`, and `postponed` to `failed_delivery`; no rollover child is minted. Future-dated postponed rows are swept into the same-night classifier, while the due-release backstop uses the same canonical status RPC. Both paths snap the row to the Lagos close date and unassign it. These policy failures are not added to **To notify** and do not count against the rep notification SLA. Currently enabled for **Shalom** only; toggle-able for any client via Catalog → Clients → Edit.
