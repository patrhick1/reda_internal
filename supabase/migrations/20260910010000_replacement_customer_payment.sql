...
alter table public.replacement_attempts
 add column customer_paid numeric not null default 0 check (customer_paid >= 0 and customer_paid < 'Infinity'::numeric),
 add column payment_method text,
 add column payment_received_by text,
 add constraint replacement_payment_valid check (
 (customer_paid=0 and payment_method is null and payment_received_by is null) or
 (customer_paid>0 and payment_method in ('cash','transfer','pos') and payment_received_by in ('rider','reda') and payment_method is not null and payment_received_by is not null));
-- Reports filter on the Lagos attempt date, not the mutable scheduled date.
create index if not exists replacement_attempts_financial_day on public.replacement_attempts (((attempted_at at time zone 'Africa/Lagos')::date));

drop function public.complete_replacement(text,uuid,jsonb,text);
CREATE OR REPLACE FUNCTION public.complete_replacement(p_client_uuid text, p_delivery_id uuid, p_return_outcomes jsonb, p_notes text DEFAULT NULL::text, p_customer_paid numeric DEFAULT 0, p_payment_method text DEFAULT NULL, p_payment_received_by text DEFAULT NULL)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
...
union all
select d.client_id,(a.attempted_at at time zone 'Africa/Lagos')::date,'replacement_attempt',a.id,a.customer_paid-a.client_charge
from public.replacement_attempts a join public.deliveries d on d.id=a.delivery_id where d.deleted_at is null;
revoke all on function public.complete_replacement(text,uuid,jsonb,text,numeric,text,text) from public,anon;
grant execute on function public.complete_replacement(text,uuid,jsonb,text,numeric,text,text) to authenticated;
revoke all on function public.correct_replacement_payment(uuid,numeric,numeric,text,numeric,text,text) from public,anon;
grant execute on function public.correct_replacement_payment(uuid,numeric,numeric,text,numeric,text,text) to authenticated;
revoke all on function public.list_replacement_financials_v2(date,date,uuid) from public,anon;
grant execute on function public.list_replacement_financials_v2(date,date,uuid) to authenticated;
revoke all on function public.list_replacement_agent_financials_v2(date,date) from public,anon;
grant execute on function public.list_replacement_agent_financials_v2(date,date) to authenticated;
revoke all on function public.list_replacement_financials_rep_v2(date,date,uuid) from public,anon;
grant execute on function public.list_replacement_financials_rep_v2(date,date,uuid) to authenticated;
