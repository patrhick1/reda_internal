...
- Gemini API + Google Maps Geocoding API for address normalization
...
**Rep.** Operational representative. Identical to dispatcher in every capability EXCEPT stock access: no stock screens, no read on `stock_adjustments`, and no place in the stock-pickup notification audience. Use this role for teammates who book and coordinate deliveries but never handle inventory or audit it. Under the hood, the rep route group `/(rep)/...` mirrors `/(dispatcher)/...` and the server-side `is_admin_or_dispatcher()` helper treats rep as part of the operational-coordinator set (with one explicit exception on the `stock_adjustments` SELECT policy).

**Agent (Rider).** Sees only their own deliveries. Updates status on those deliveries. Records payment collected. Sees their own stock in hand. Sees their own per-delivery and per-period earnings. Cannot see margin or Reda's charge. Cannot see other agents' work.

**Warehouse.** Stock holder used in transfers (sits on the receiving side of vendor intakes and on the issuing side when agents pick up). Has a minimal app surface — Stock + Profile tabs only.

**Key access rule:** margin (Reda's profit per delivery) is admin-only. Agents, dispatchers, and reps should not see what Reda earns on each delivery. Enforced at the API + database level, not just UI.
...
1. **Google Maps Geocoding API.** Send the raw address. Maps returns coordinates + structured place names (e.g., "Iganmu, Lagos, Nigeria").
2. **Gemini (if needed).** Send Maps' output + the rate card location list to Gemini. Gemini returns:
   ```json
   {
...
- **Sibling match** is conservative on purpose: same scheduled_date + normalized phone + product, AND either (a) identical `bot_raw_message` text (handles AI-parser drift across bot-forwarded duplicates) OR (b) identical address + quantity (manual duplicates). Same-day repeat orders for the same customer don't false-cancel because their address or message text differs.
