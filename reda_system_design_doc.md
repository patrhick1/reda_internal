...
**Warehouse.** The warehouse-role user **is the physical place where stock is stored** — one identity per location (Shomolu warehouse today; future locations would be additional warehouse-role users). Originally every floor hand shared that one login, so the audit recorded every action as the warehouse identity — per-human accountability collapsed into a single books-of-record actor. **Since 2026-06-06 that collapse is optional: staff can log in as themselves while still acting on the one shared set of books (see "Warehouse staff" below).**
...
**Warehouse staff — one warehouse, named operators (since 2026-06-06).** The warehouse role now comes in two flavours, distinguished by a nullable `users.warehouse_id`:
- **Place** (`warehouse_id IS NULL`) — the stock holder; the books. Shomolu today. `current_stock` keys on the place, so all warehouse stock lives on one balance.
- **Staff** (`warehouse_id` set) — a person who logs in as themselves and acts on the linked place's books. Staff hold no stock of their own.

The two stock RPCs' self-only guard (`p_agent_id = v_actor`) became `coalesce(v_warehouse_id, v_actor)`, so a staffer's intake / issue / return / shrinkage lands on the **place's** books while `created_by_user_id` and the audit log record the **real person** — multiple people manage one warehouse, one set of books, with per-person accountability. Admin onboards a staffer via Catalog → Users → New user → role **Warehouse** → **"Belongs to warehouse"** (leave empty to create a new place instead). RLS on `stock_adjustments` lets staff read/write their place's rows; the `warehouse_pickup` push and `current_stock` are unchanged (staff contribute zero stock). Backward-compatible — the existing Shomolu user keeps `warehouse_id = NULL` and behaves exactly as before, and the shared login still works as a fallback. SQL: [scripts/warehouse-staff.sql](scripts/warehouse-staff.sql).
...
- **Rollover dedup (revised 2026-06-02).** The 23:59 auto-rollover **collapses every sibling group to a single canonical** before rolling forward. Same-agent dupes collapse to one row per agent; cross-agent groups (race-assigns) collapse to one canonical regardless of progression state. The collapse is correct *because rolled rows land unassigned* (see "End of day" below) — preserving multiple parents through EOD would just spawn multiple unassigned phantoms tomorrow, forcing Uzo to dedup by eye in the morning queue. Uzo can re-race fresh tomorrow morning by reassigning to multiple agents again. Prior behavior (2026-05-18 → 2026-06-02) preserved all-pending cross-agent siblings through rollover; that was the right policy when rollovers inherited parent assignments, but stopped making sense after the 2026-05-30 unassigned-rollover change. Separately, the rollover also runs a **resolved-sibling backstop** before rolling each live row forward — if a sibling already reached a terminal state that settled the order, the live row is cancelled rather than rolled. That backstop must use the **same `{agent_cancelled, rolled_over}` exclusion as the live cascade** (fixed 2026-06-04 — see §3.5); a `cancelled` sibling settles the order and cascades, but an `agent_cancelled` or `rolled_over` sibling does not and must never cancel a live row.
...
**The cascade-exclusion list must be mirrored everywhere that reasons about "has a sibling already resolved this order" — not just in the live trigger (fixed 2026-06-04).** The live `tg_handle_sibling_coordination` excludes `{agent_cancelled, rolled_over}`. But `run_eod_rollover` has its own backstop — a `resolved` lateral join that, before rolling a live row forward, checks whether any **terminal** sibling already settled the order and, if so, cancels the live row (*"Another agent already handled this order (…). Closed as duplicate."*). That backstop matched on `category='terminal'` with **no exclusions**, so it treated `agent_cancelled` and `rolled_over` siblings as "handled" — the exact opposite of the live cascade. Consequence on the night of 2026-06-04: the 2026-06-03 phantom-race cleanup had closed phantom rows with plain `cancelled` (it should have used `agent_cancelled`), the cascade took the intended-agent rows down with them, the operator restored those intended rows to `pending`, and ~3.5 h later the rollover's backstop re-cancelled all of them — reading the dead phantom sibling as "another agent handled it" — so **9 real orders rolled nothing forward and vanished from the next day's queue.** Two-part fix: (a) **`agent_cancelled` is the mandated status for any per-row close that must not end the order** — phantom cleanup, "not my delivery", admin removing one row of a sibling pair; the bot root cause that *created* phantoms was already fixed 2026-06-03, this is the rule for the manual cleanup path that used `cancelled`; (b) `run_eod_rollover`'s resolved-sibling join now excludes `{agent_cancelled, rolled_over}`, identical to the live cascade, so `agent_cancelled` behaves as non-cascading at rollover time too ([scripts/fix-rollover-resolved-sibling-match-cascade.sql](scripts/fix-rollover-resolved-sibling-match-cascade.sql)). `cancelled` is deliberately **kept** as a resolving terminal in both places — a cancelled order is dead and should close its siblings everywhere, consistently. The 9 lost orders were restored to the next operating day as `pending`, unassigned ([scripts/restore-june4-lost-rollover-orders.sql](scripts/restore-june4-lost-rollover-orders.sql)). Rule of thumb: any new code that branches on "a sibling reached a terminal state" must use the same `{agent_cancelled, rolled_over}` exclusion, or it will silently diverge from the cascade.
...
**Carry-cap only counts genuine soft-fails (since 2026-05-30).** The 2-strike rule (mark `unserious` after 1 rollover — original day + one follow-up day, lowered from 2 on 2026-06-20) now fires only when the parent's status was a customer-unreachable one — `not_answering`, `not_around`, `not_available`, `not_connecting`, `number_busy`, `switched_off`. Operational rollovers (a row that sat as `pending` because nobody assigned it, an `available` row that never got attempted, a `picked_up` row the agent didn't deliver) carry without burning a strike. Captured per-rollover in `audit_log` as `is_strike_rollover: true|false`.
...
flowchart TD
  ...
  ES3 -->|no| ES4{"rollover_count >= 1 soft-fail?"}
...
Surfaced by the 2026-06-05 workflow audit (and extended by the 2026-06-08 permissions/RLS pass); each verified against deployed code and the live DB (not inferred from this doc, not from the `scripts/*.sql` files). Ordered by impact. **Three were fixed and deployed on 2026-06-08 — Tier S (S1+S2), Tier A #1, and Tier A #4 — see the ✅ RESOLVED tags below; the rest are not committed work yet.**

**Tier S — enforcement / security holes (permissions + RLS audit 2026-06-08) — ✅ RESOLVED & DEPLOYED 2026-06-08**

These don't break a workflow's *logic* — they mean the database doesn't actually *enforce* the logic the RPCs implement. The state machine, stock guard, location-required guard, and history append are all correct **inside** the `SECURITY DEFINER` functions, but the base tables are directly writable by ordinary logged-in users, so a determined agent can skip the functions entirely. Verified live on 2026-06-08 via `pg_policies`, `information_schema.column_privileges`, and the `deliveries` / `delivery_status_history` grants — not from the app and not from the SQL scripts.

S1. **Agents can mutate their own `deliveries` rows directly, bypassing every RPC check.** `authenticated` holds table-level (all-column) `UPDATE` on base `public.deliveries`, and RLS `deliveries_update_own_or_admin` is `USING (is_admin_or_dispatcher() OR assigned_agent_id = auth.uid())` with **`WITH CHECK = NULL`** (so the `USING` predicate governs writes too). A `PATCH /rest/v1/deliveries?id=eq.<own-row>` therefore lets the assigned agent set `current_status`, `paid`, `quantity_delivered`, `charged_snapshot`, `agent_payment_snapshot`, `cash_pos_fee_snapshot`, `assigned_agent_id`, `location_id`, … with **none** of the guards `change_delivery_status` enforces: no `delivery_status_transitions` validity check, no stock guard, no location-required guard, no `requires_admin` gate on terminal/backward moves, and **no `delivery_status_history` row** (so the change is invisible in the timeline and `audit_log`). The reconciliation RPCs read these same columns, so this is a financial-integrity hole, not just a status one. The app's screens never expose this; it takes a hand-crafted REST call with the agent's own JWT — but the protection today is "please use the app," not a lock. **Severity: High.** Fix: `REVOKE INSERT, UPDATE, DELETE ON public.deliveries FROM authenticated, anon` (force every write through the `SECURITY DEFINER` RPCs), or replace the permissive UPDATE policy with a tight `WITH CHECK` + per-column grants. *Evidence: `pg_policies(deliveries)`, `information_schema.column_privileges(deliveries, authenticated)`.* — **✅ RESOLVED & DEPLOYED 2026-06-08:** `scripts/lockdown-base-table-writes.sql` revoked INSERT/UPDATE/DELETE/TRUNCATE on `deliveries` from `authenticated`/`anon` and replaced `deliveries_update_own_or_admin` with admin/dispatcher-only `deliveries_update_admin_dispatcher`. `change_delivery_status` (postgres-owned `SECURITY DEFINER`) is unaffected and remains the only write path. Verified: 0 stray write grants remain; `scripts/smoke-launch-fixes.sql` §0a/§0b/§1 pass.

S2. **Agents can insert fabricated `delivery_status_history` rows.** The append-only timeline is correctly protected against UPDATE/DELETE (`trg_status_history_no_update` / `trg_status_history_no_delete`), but `authenticated` *also* holds `INSERT`, and `dsh_insert_scoped`'s `WITH CHECK` permits the assigned agent to insert rows for their own delivery with **no validation** that `to_status` is a legal transition or matches `deliveries.current_status`. An agent can plant a *"delivered · paid in full"* line that never happened; it sits in the official, "immutable" log looking legitimate. Same root cause and same fix as S1 (remove the direct write grant; route through `change_delivery_status`). **Severity: Medium-Low** (pairs with S1). *Evidence: `pg_policies(delivery_status_history)` INSERT check + `authenticated` write grants.* — **✅ RESOLVED & DEPLOYED 2026-06-08:** same script revoked INSERT/UPDATE/DELETE on `delivery_status_history` (SELECT preserved — the timeline read still works) and replaced `dsh_insert_scoped` with admin/dispatcher-only `dsh_insert_admin_dispatcher`.

**Scope note (verified clean 2026-06-08):** the same broad `authenticated` write grants exist on most base tables, but RLS *does* defend the rest — `users` UPDATE is `is_admin()`-only (**no agent self-promotion to admin**), and `clients` / `rate_card` / `agent_profiles` / `locations` / `product_catalog` / `stock_adjustments` writes are all admin-RLS-gated. `deliveries` (S1) and `delivery_status_history` (S2) are the only two base tables whose RLS lets a non-ops user through. One revoke pattern closes both.

**Tier A — active integrity/financial holes**

1. **✅ RESOLVED & DEPLOYED 2026-06-08 — Partial-delivery leftovers never auto-return to stock.** `current_stock` subtracts only `quantity_delivered`; the unsold `(ordered − delivered)` stays on the agent's books indefinitely. `MarkDeliveredSheet` captured only qty/paid/method — there was no return-to-warehouse step, despite §5 claiming "agent's choice at delivery time." → agent inventory silently inflated; real shrinkage masked. *Evidence: `current_stock` view def + the mark-delivered sheet fields.* **Fix shipped:** new `return_delivery_leftover` RPC (`scripts/return-delivery-leftover.sql`) + an opt-in *"Return N to warehouse"* toggle on `MarkDeliveredSheet` (shown only on partials, offline-queued via the `return_delivery_leftover` job kind) hands the leftover back as a paired `warehouse_return`. Default still keeps it with the agent (the §5 "agent's choice"). Smoke §2 passes (agent 8→5, warehouse +3, idempotent, over-return rejected). **Forward-only:** the ~102 historically-stranded units are a separate ops backfill.
...
3. **~~Customer underpayment has no collection/aging workflow.~~ NOT A GAP — withdrawn 2026-06-08 (business rule).** There is no "underpayment owed" concept by design: **the vendor pre-agrees the price Reda collects from the customer; whatever is collected is final, and any difference is strictly between the customer and the vendor — never a debt owed to Reda or to track.** So no per-customer tab, aging, or follow-up is wanted. `outstanding = customer_price − paid` (in `client_remit_summary` and the reconcile UI) is therefore at most an **informational nominal delta, not a receivable** — open question whether to relabel/drop it so it doesn't read as money owed. The `Remit = paid − charged − cash_pos_fee` mechanic (compute from actual `paid`) stays correct and is the right defensive default.

4. **✅ RESOLVED & DEPLOYED 2026-06-08 — An *unassigned* row marked delivered slips every net.** The mark-delivered stock guard ran only `if assigned_agent_id is not null`, and `current_stock`'s decrement requires `assigned_agent_id IS NOT NULL`. So a delivered row with no agent: no stock check, no stock decrement, no rider credited. *Evidence: guard clause + view `WHERE assigned_agent_id IS NOT NULL`.* **Fix shipped:** `change_delivery_status` now raises `no_agent_assigned` (P0001) when a row with no `assigned_agent_id` is transitioned to `delivered` (`scripts/fix-delivered-requires-agent.sql` — reproduced verbatim with one inserted guard; covers single, bulk-agent, and bulk-status paths). Smoke §1 passes (blocked when unassigned, delivers once assigned+stocked). **Forward-only:** the 8 pre-existing unassigned-delivered rows need a separate review.
...
6. **The agent-bonus snapshot is a loaded-but-unloaded gun.** `effective_rate` adds `agent_payment_bonus` only when an agent is passed. `auto_assign_delivery` **and** `update_delivery_fields` (the ops Edit-screen reassign) both re-snapshot *with* the agent, so those paths are correct. But `bulk_assign_deliveries`, `reassign_to_sub_agent`, and post-rollover reassignment do **not** — `bulk_assign` updates only `assigned_agent_id`, and `rollover_delivery` re-snapshots via `current_rate_for_location` with no agent (rows land unassigned). Harmless today — **0 of 21 agents have a non-zero bonus (verified in DB)** — but the moment anyone sets one, the dominant *rollover → unassigned → bulk-assign* path underpays, and since rollover forces manual reassignment, that's a large share. *(Corrected 2026-06-08: the prior note said "only `auto_assign` re-snapshots"; the Edit-screen path does too.)*
...
8. **~~No post-delivery return/refund path.~~ WON'T DO — out of Reda's scope by design (confirmed 2026-06-08).** Post-delivery returns/refunds are handled **customer ↔ vendor directly — the customer calls the vendor; Reda has no role.** Not a Reda workflow at all (more than just deferred). The blunt admin `revert delivered` remains only for genuine mark-delivered mistakes, not returns.

9. **Operational/config brittleness:** cash POS fee hardcoded ₦500 (bank-fee change = code change); two parallel bot pipelines (`bot_inbound` prod + `mybot_inbound` shadow) to keep in sync. (The earlier "warehouse shared login = no per-human shrinkage attribution" gap was **resolved 2026-06-06** — warehouse staff now log in as themselves; see §2 "Warehouse staff".)

Suggested first moves: ~~#1 and #4~~ **done 2026-06-08** (plus the Tier S write-bypass); next up — **#2** is what turns retroactive corrections risky at scale; **#5** unblocks agents at the door; **#6** is cheap insurance to apply before anyone ever sets a bonus.

---

*Last updated: 2026-05-19. §12 added when voice calling shipped on top of v1. 2026-06-05: stale-fact pass on §4/§6/§7 (transfer default, remit formula, view-only stock decrement, status categories), plus §13 end-to-end workflow map and §14 known-gaps from the workflow audit. 2026-06-08: §14 Tier S (base-table write bypass — `deliveries` direct UPDATE + `delivery_status_history` direct INSERT) added from the permissions/RLS audit; §14-6 corrected (`update_delivery_fields` does re-snapshot the bonus). All §14 findings verified against the live DB, not the `scripts/` files. 2026-06-08 (same day): Tier S, #1, and #4 fixed + deployed to Supabase (backing SQL gitignored under `scripts/`) and verified by `scripts/smoke-launch-fixes.sql` — marked ✅ RESOLVED in place. 2026-06-08: §14-3 withdrawn (no underpayment-owed concept — vendor pre-agrees the collected price; any difference is customer↔vendor, never Reda's) and §14-8 marked won't-do (returns are handled customer↔vendor directly; Reda has no role) per business clarification.*
