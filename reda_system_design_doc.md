...
Surfaced by the 2026-06-05 workflow audit (and extended by the 2026-06-08 permissions/RLS pass); each verified against deployed code and the live DB (not inferred from this doc, not from the `scripts/*.sql` files). Ordered by impact. These are tracked here so they're not lost in chat — none are committed work yet.

**Tier S — enforcement / security holes (added 2026-06-08, permissions + RLS audit)**
