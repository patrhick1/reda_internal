...
**Rep.** Operational representative. Identical to dispatcher in every capability EXCEPT stock access: no stock screens, no read on `stock_adjustments`, and no place in the stock-pickup notification audience. Use this role for teammates who book and coordinate deliveries but never handle inventory or audit it. Under the hood, the rep route group `/(rep)/...` mirrors `/(dispatcher)/...` and the server-side `is_admin_or_dispatcher()` helper treats rep as part of the operational-coordinator set (with one explicit exception on the `stock_adjustments` SELECT policy). Reps are the **intermediary layer between clients (vendors) and agents (riders)** — they take the operational heat from clients, coordinate with agents on the ground, and shield agents from any direct vendor relationship.
