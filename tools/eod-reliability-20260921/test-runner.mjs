...
if (race) {
  ...
  sql("select reda_maintenance.enqueue('release',reda_maintenance.business_day()); select reda_maintenance.dispatch();");
  ...
  const agent=concurrent("BEGIN; select set_config('request.jwt.claim.sub',md5('eod-test-agent')::uuid::text,true); select public._same_customer_lock_orders(array[md5('eod-test-order-1')::uuid]); select 'LOCK_READY'; select pg_sleep(2); update deliveries set current_status='available' where id=md5('eod-test-order-1')::uuid; COMMIT;");
  ...
  for (const name of ['test-maintenance.sql','test-reschedule-failures.sql','test-groups-permissions.sql']) {
    const result = execFileSync(psql,[...args,'-f',path.join(dir,name)],{encoding:'utf8',stdio:['pipe','pipe','pipe']});
    console.log(`PASS ${name} (${result.trim().split('\n').at(-1)})`);
  }
  ...
  for (const scale of [1,10,50]) {
    ...
    sql("select reda_maintenance.enqueue('release',reda_maintenance.business_day()); select reda_maintenance.enqueue('close',reda_maintenance.business_day()-2);");
    ...
    writeFileSync(path.join(dir,'load-results.json'),JSON.stringify(results,null,2)+'\n');
