...
const today = '2026-09-21', target = '2026-09-22';
...
const calls = [], errors = [];
const preview = { preview_id: '44444444-4444-4444-8444-444444444444', kind: 'finish_day', date: today, target_date: target, total_orders: 101, oversized_groups: 0, summary: { roll: 100, cap_unserious: 1 }, rows: Array.from({length:100}, (_, i) => ({ id: `row-${i}`, customer_name: `TEST review ${i}`, status: 'pending', date: today, agent: 'TEST Agent', carry: 0, action: 'roll', target_date: target })) };
...
try {
  ...
  await page.getByRole('button',{name:'Preview next working day',exact:true}).click();
  await page.getByText(`${today} → ${target}`,{exact:true}).waitFor();
  await page.getByRole('button',{name:'Load more reviewed orders',exact:true}).click();
  ...
  await page.getByRole('button',{name:`Finish day · prepare ${target}`,exact:true}).click();
  await page.getByText('1 groups remaining · 0 failed · 0 skipped after changes',{exact:true}).waitFor();
  ...
  for (const tab of ['By client','By agent','Summary']) {
    ...
    await page.getByRole('button',{name:'Open end of day',exact:true}).click();
    await page.getByRole('button',{name:'Preview next working day',exact:true}).waitFor();
  }
